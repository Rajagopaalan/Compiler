...
struct typeNode {
	char* id;
	struct tokensNode* head;
	struct tokensNode* curr;
};

struct typeNode types[MAX_tokens];
...
struct tokens {
	...
	char* declType; // EXPLICIT or IMPLICIT
         
   
	int typeNum; // Type Number for checking
	char* format; // "type_decl" or "var_decl"
};

// Declare Array to Store tokenss
struct tokens tokenlist[MAX_tokens];
...
void semantic_error(int error,char* token) {
	printf("ERROR CODE %d %s\n", error,token);
}
void sem_error(int error,int line) {
...
int unify(int t1, int t2) {
	...
	if ((t1 < 14) & (t2 < 14)) {
		...
		// Loop through tokenss
		for (i = 0; i < tokensCount; i++) {
			// Check for Matching Type Number
			if (tokenlist[i].typeNum == t2) {
				// Assign Type Number for tokens
				tokenlist[i].typeNum = t1;
			}
		}
	}
...
int checktokens(char* id, char* declType, char* format, int code, int typeNum) {
	int i;
	int found = FALSE;

printf("%s",id);
	// Loop through tokens Table Return Type Number
	for (i = 0; i < tokensCount; i++) {
		if (strcmp(id, tokenlist[i].id) == 0) {
			// ID found in tokens Table
			found = TRUE;
			...
			switch (code) {
				case TYPE_NAME:
					// Type Name Search
					// Return Type Number
                                    if(tokenlist[i].format == "var_decl") {
						// Variable declared more than once
						semantic_error(4 ,id);
					}
					return tokenlist[i].typeNum;
				case TYPE_ID:
					// Type Declaration ID List Search
					// ID found while Error checking
					// Create Semantic Error 0
                                      if(tokenlist[i].format=="var_decl"){semantic_error(4,id);}
					else{semantic_error(0,id);}
					exit(0);
				case VAR_ID:
					// Var Declaration ID List Search
					// ID found while Error checking
					// Create Semantic Error 1 or 2
					if (tokenlist[i].format == "type_decl") {
						// Type re-declared as Variable
						semantic_error(1,id);
					} else if (tokenlist[i].format == "var_decl") {
						// Variable declared more than once
						semantic_error(2,id);
					} else {
						// Should Never Reach Here
						// Only will occur if tokens not inserted correctly
						printf("ERROR UNKNOWN tokens format");	
					}
					exit(0);
				case COND_ID:
					...
					if (tokenlist[i].typeNum != BOOLEAN) {
						if (tokenlist[i].typeNum < BOOLEAN) {
							// Type Mismatch
							// Throw ERROR CODE 3
							sem_error(3,line_no);
							...
							int tempTypeNum = tokenlist[i].typeNum;
							tokenlist[i].typeNum = BOOLEAN;
							return tempTypeNum;
						}
					}
	...
	// Insert ID into tokens Table if not found
	if (!found) {
		// ID not found
		...
		newtokens.id = id;
		newtokens.declType = declType;
		newtokens.format = format;
		newtokens.typeNum = typeNum;

		// Insert into tokens Table
		tokenlist[tokensCount] = newtokens;
...
void lookup_type(struct type_declNode* typeDecl) {
	...
	if (typeDecl->type_name->type == ID) {
		// If Type Name is ID then lookup in tokens Table
		typeNum = checktokens(typeDecl->type_name->id, "IMPLICIT", "type_decl", TYPE_NAME, nextTypeNum);
	} else {
	...
	while (id_listNode != NULL) {
		checktokens(id_listNode->id, "EXPLICIT", "type_decl", TYPE_ID, typeNum);	// Lookup ID in tokens Table
...
void lookup_var(struct var_declNode* varDecl) {
	...
	if (varDecl->type_name->type == ID) {
		// If Type Name is ID then lookup in tokens Table
		typeNum = checktokens(varDecl->type_name->id, "IMPLICIT", "type_decl", TYPE_NAME, nextTypeNum);
	} else {
		// Else it is a Built-In Type so assign to appropriate Type Number
	...
	// Loop through IDs in ID List
	while (id_listNode != NULL) {
		checktokens(id_listNode->id, "EXPLICIT", "var_decl", VAR_ID, typeNum);	// Lookup ID in tokens Table
		id_listNode = id_listNode->id_list;	// Iterate to next node in ID List
	}
...
int lookup_expr(struct exprNode* expr) {
	...
	if (expr->tag != PRIMARY) {
		...
		if (expr->primary->tag == ID) {
			...
			return checktokens(expr->primary->id, "IMPLICIT", "var_decl", STMT, nextTypeNum);
		} else if (expr->primary->tag == NUM) {
			// Rule 40 (primary -> NUM)
			// Return INT (10)
			return INT;
...
void lookup_assign(struct assign_stmtNode* stmt) {
	...
	idTypeNum = checktokens(stmt->id, "IMPLICIT", "var_decl", STMT, nextTypeNum);
