...
if(turn=="TYPE_NAME" && tl[i].ttype == "var_decl" ){
     ...
     else   if((turn=="VAR_ID" ||turn=="COND"  ||turn=="COND_PRI"  ||turn=="STMT") && tl[i].ttype == "type_decl")
	{
						err(1,tn);
						exit(0);
					}		
...
if(tl[i].dt==INT || tl[i].dt == REAL || tl[i].dt == LONG || tl[i].dt==STRING){                           
      ...
      if (turn=="TYPE_NAME" || turn=="COND_PRI" || turn=="STMT" ||  (turn=="COND" && tl[i].dt!=BOOLEAN)) {
return tl[i].dt;}
...
temps=checktokens(switch_Node->id, "type_decl", INT,"COND_PRI");
